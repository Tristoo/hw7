      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          ...
          infowindow.open(map, marker);
			cnt++;
		alert("Here's the longitude and latitude of " + locations + "\nThe amount of markers clicked/shown so far is: " + cnt);
        }
      })(marker, i));
    }
	...
	for (d = 0; d < clearihue.length; d++) {  
      ...
      google.maps.event.addListener(marke, 'click', (function(marke, d) {
        return function() {
		  ...
		  cnt ++;
		alert("Here's the longitude and latitude of " + clearihue + "\nThe amount of markers clicked/shown so far is: " + cnt);
        }
      })(marke, d));
    }
	...
	for (j = 0; j < fel.length; j++) {  
      ...
      google.maps.event.addListener(mark, 'click', (function(mark, j) {
        return function() {
		  ...
		  cnt ++;
		alert("Here's the longitude and latitude of " + fel + "\nThe amount of markers clicked/shown so far is: " + cnt);
        }
      })(mark, j));
    }
